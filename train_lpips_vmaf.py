...
if __name__ == '__main__':
    ...
    ### Export directory
    folder_run = f"VMAF&LPIPS_CRF:{crf}_W0:{w0}_W1:{w1}_W2:{w1}"
    ...
    if args.WB_NAME:
        import wandb
        name_run = f"VMAF&LPIPS_CRF:{crf}_W0:{w0}_W1:{w1}_W2:{w2}"
        tag_run = "VMAF&LPIPS"
        wandb.init(
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % args.VALIDATION_FREQ == 0:
            ...
            generator_path = os.path.join(
                args.EXPORT_DIR, 
                f"{arch_name}_epoch:{epoch}_ssim:{ssim_mean:.4f}_vmaf:{vmaf_mean:.4f}_lpips:{lpips_mean:.4f}_crf:{crf}.pth"
            )
